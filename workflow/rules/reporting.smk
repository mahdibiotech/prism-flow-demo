rule render_report:
    ...
    shell:
        ...
        PRISM_FLOW_ROOT=$(pwd) quarto render workflow/report/report.qmd \
          --output-dir $(pwd)/results/report \
          --output prism_flow_report.html > {log:q} 2>&1
        END=$(date -u +%Y-%m-%dT%H:%M:%SZ)
        python scripts/write_provenance.py \
          --event provenance/events/render_report.json \
          --rule render_report --step reporting --status success \
          --started "$START" --finished "$END" \
          --inputs {input.validation:q} {input.qc:q} {input.de:q} \
          --outputs {output:q} \
          --command "PRISM_FLOW_ROOT=$(pwd) quarto render workflow/report/report.qmd" \
